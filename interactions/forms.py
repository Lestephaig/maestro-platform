...
class InteractionForm(forms.ModelForm):
    event_type = forms.ChoiceField(
        choices=Interaction.EVENT_TYPE_CHOICES,
        required=False,
        label='Тип мероприятия',
        widget=forms.Select(attrs={'class': 'form-select'}),
    )
    event_format = forms.ChoiceField(
        choices=Interaction.EVENT_FORMAT_CHOICES,
        required=False,
    ...
    class Meta:
        ...
        fields = [
            ...
            'has_green_rooms',
            'staff_host',
            'staff_sound_engineer',
            'staff_light_engineer',
            'staff_tech_admin',
            'staff_security',
            'staff_medical',
            'info_promo_materials',
            'info_online_registration',
            'info_photo_video',
            'info_live_stream',
            'paid_entry',
            'ticket_price',
            'venue_special_requirements',
            'additional_comments',
            'start_date',
            'end_date',
            'budget_amount',
        ...
        widgets = {
            ...
            'venue_requirements': forms.Textarea(attrs={'rows': 3}),
            'venue_special_requirements': forms.Textarea(attrs={'rows': 3}),
